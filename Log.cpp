...
Log::Log() : logMode(LOG_MODE_FILE), dialogHandle(0), listHandle(0)
{
	...
	if (SHGetFolderPathA( NULL, CSIDL_LOCAL_APPDATA, NULL, 0, szPath ) != S_OK)
	{
		strFilePath = "ArmadaTrace.txt";
	}else
	{
		PathAppend(szPath, TEXT("Temp\\ArmadaTrace.txt"));
	}
	
	strFilePath = szPath ;

	logFile.open(strFilePath.c_str(), std::ios::out);
	if(logFile.fail())
		return;
...
void Tracelog(const wchar_t *format, ...)
{
    ...
    _vsnwprintf_s(buffer, LOG_MAX_STRING, format, valist);
    va_end(valist);
...
void Tracelog(const std::wstring& str)
{
